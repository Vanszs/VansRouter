...
function clientSession(sessionId) {
  ...
  return CANONICAL_SESSION.test(value) ? value : "";
}

// Canonical id formats OpenCode validates: ses_<12 hex><14 base62> / msg_<12 hex><14 base62>.
// Ported from upstream decolua/9router 6091ff597.
...
export class OpenCodeExecutor extends BaseExecutor {
  ...
  buildHeaders(credentials, stream = true) {
    ...
    return {
      ...
      "x-opencode-session": clientSession(raw["x-opencode-session"]) || credentials?.runtimeOpencodeSession || generateSessionId(),
