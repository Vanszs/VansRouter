 ...
 *
 * A Qoder model_config advertises a `context_config` list (200K / 400K / 1M for
 * qmodel_38max) while `max_input_tokens` only carries the tier the IDE currently
 * has selected (~180K by default). The Qoder IDE lets the user switch tiers from
 * the model picker; a qodercli-style client (which is what we impersonate) has no
 * picker, so a long Claude-Code / Codex session that grew past the default tier is
 * rejected upstream even though the model itself supports 1M.
 *
 * This module emulates the IDE: estimate the prompt size, pick the smallest
 * advertised tier that fits (never below the model's current default), and mirror
 * the choice into the same three places the IDE writes:
