...
export async function getChartData(period = "7d") {
  ...
  const startDate = period === "all" ? new Date(`${firstKey}T00:00:00`) : new Date(today);
  if (period !== "all") startDate.setDate(startDate.getDate() - (bucketCount - 1));
  const dayCount = Math.max(1, Math.round((today - startDate) / 86400000) + 1);
