              ...
              <div className="flex flex-col gap-2">
                ...
                {tool.notes && tool.notes.length > 0 && (
                  <div className="flex flex-col gap-2 mb-2">
                    {tool.notes.map((note) => (
                      <div key={`${note.type}-${note.text}`} className={`flex items-start gap-2 p-2 rounded text-xs ${
                ...
                <div className="grid grid-cols-1 gap-1.5 sm:grid-cols-[8rem_auto_1fr_auto] sm:items-center sm:gap-2">
                  ...
                  <div className="relative w-full min-w-0">
                    <input type="text" value={selectedModel} onChange={(e) => setSelectedModel(e.target.value)} aria-label="Model ID" placeholder="cc/claude-opus-4-7" className="w-full min-w-0 pl-2 pr-7 py-2 bg-surface rounded border border-border text-xs focus:outline-none focus:ring-1 focus:ring-primary/50 sm:py-1.5" />
                ...
                <div className="flex flex-col gap-1 p-3 bg-blue-500/5 border border-blue-500/20 rounded-lg">
                  ...
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile VansRoute</code>
                  <code className="text-xs font-mono text-text-muted">jcode --provider-profile VansRoute --model {selectedModel || "cc/claude-opus-4-7"}</code>
                </div>
              </div>
...
export default function JcodeToolCard({
  ...
  const jcodeStatus = state.status;
  const checkingJcode = state.checking;
