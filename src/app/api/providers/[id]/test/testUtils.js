...
const OAUTH_TEST_CONFIG = {
  ...
  github: {
    ...
    authHeader: "Authorization",
    authPrefix: "Bearer ",
    extraHeaders: { "User-Agent": "VansRouter", "Accept": "application/vnd.github+json" },
  },
  iflow: {
...
async function testApiKeyConnection(connection, effectiveProxy = null) {
  ...
  try {
    switch (connection.provider) {
      ...
      case "kimchi": {
        const res = await fetchWithConnectionProxy("https://llm.kimchi.dev/openai/v1/models", { headers: { Authorization: `Bearer ${connection.apiKey}` } }, effectiveProxy);
        return { valid: res.ok, error: res.ok ? null : "Invalid API key" };
      }
      case "perplexity": {
