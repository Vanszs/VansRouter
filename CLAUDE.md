...
- Security-sensitive env: `JWT_SECRET` (session cookie), `INITIAL_PASSWORD` (no production default; set a strong value before first login), `API_KEY_SECRET`, `MACHINE_ID_SALT`. Full env contract in `.env.example` and ARCHITECTURE.md's env matrix.
