...
VansRouterに関する一般的な質問。

---

## VansRouterとは?

**VansRouterは、サブスクリプションの価値を最大化し、コストを最小限に抑えるAIモデルルーターです。**
...
**はい、VansRouter自体は100%無料でオープンソースです。**
...
**VansRouterはすべてのプロバイダーのクォータをリアルタイムで追跡します:**
...
## VansRouterをセルフホストできますか?

**はい! VansRouterは複数のデプロイメントオプションをサポートします:**
...
- `INITIAL_PASSWORD` - ダッシュボードログイン（新規インストールは `123456`、公開前に強いランダム値を設定）
...
**はい、VansRouterはセキュリティとプライバシーを優先します:**

**ローカルストレージ:**
- すべてのデータは`~/.9router`(またはカスタム`DATA_DIR`)にローカル保存
- VansRouterサーバーへのデータ送信なし
- OAuthトークンはJWTで暗号化
...
**破壊的変更:**
- [CHANGELOG.md](https://github.com/Vanszs/VansRouter/blob/main/CHANGELOG.md)を確認
...
詳細は[CONTRIBUTING.md](https://github.com/Vanszs/VansRouter/blob/main/CONTRIBUTING.md)を参照。

---

## さらにヘルプが必要?

- **ドキュメント:** [VansRouter docs](https://github.com/Vanszs/VansRouter/blob/main/README.md)
