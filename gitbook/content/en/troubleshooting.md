...
Common issues and solutions when using VansRouter.
...
1. **Auto-refresh (default):**
   VansRouter automatically refreshes tokens. Wait 30 seconds and retry.
...
1. **Check if VansRouter is running:**
   ```bash
   # Check process
   ps aux | grep vansrouter
