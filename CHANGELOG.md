...
- **Fast Docker multi-arch build** — Runs Next.js build natively on host CPU (`--platform=$BUILDPLATFORM`) and isolates native C++ compilation to the target platform, dropping multi-arch build duration from 60+ minutes (QEMU timeout) to ~4-5 minutes. Decoupled npm publishing from Docker build so npm packages publish immediately.
