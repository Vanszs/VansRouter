...
- **Standalone self-containment now finds Windows junctions** — `collectSymlinks` only recognised `isSymbolicLink()`, but a junction is reported as a directory, so the walk descended *through* it into the build tree the pass exists to detach from and returned zero links. `makeReleaseSelfContained` therefore reported success on Windows while leaving every pnpm link pointing back into the build directory that deploy-atomic then deletes, which is the exact failure the function was written to prevent. A link is now identified by `readlink` succeeding, which returns a target for a link of either kind and throws `EINVAL` for a real directory, and link removal goes through a shared `removeLink` that picks `rmdir` or `unlink` from the node's real shape rather than a platform guess — so a simulated-Windows run, which links a POSIX symlink on Linux, still takes the right branch. An inode comparison (`lstat` vs `stat`) was tried first and rejected: NTFS can report the same index for a reparse point and its target, so it would have kept returning zero on the platform it was written for.
