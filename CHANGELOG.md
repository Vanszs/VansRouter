...
- **Duplicated version literals collapsed into existing constants** — `anthropic-version: "2023-06-01"` appeared 14 times across 8 files while `ANTHROPIC_API_VERSION` already sat in `providers/shared.js` described as the "single source". All fourteen now reference it; the value is identical at every site, so behaviour cannot change. Cursor's client version was a genuine latent bug rather than plain duplication: `cursorChecksum.js` hardcoded `3.13.25` while `registry/cursor.js` carried a separate `clientVersion` that *is* read by `src/lib/oauth/services/cursor.js`, so bumping the registry left the checksum path sending the old version. The commit hash existed only in that hardcoded copy. Both are now `CURSOR_IDE_VERSION` / `CURSOR_IDE_COMMIT`.
