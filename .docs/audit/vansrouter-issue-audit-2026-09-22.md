...
**Pending upstream/fork PR #130** (`fix(db): stop renaming healthy data.sqlite when the sqlite driver fails
to load`, `its-benjamin`, open, `MERGEABLE`/`CLEAN`, 4 files): it replaces the driver-load-as-corruption
assumption — today a `better-sqlite3` load failure makes a *healthy* DB look corrupt and gets it renamed and
overwritten from backup. Verified locally by applying the PR's 4 files to this tree and running its tests:
`tests/unit/db-corruption-recovery.test.js` 9 passed + `tests/unit/db-webpack-runtime.test.js` 1 passed
(10 passed, files then restored; my changes and the PR touch disjoint files). Effect if merged: fail-closed
stays, but **auto-restore from `db/backups/` is removed** (`BACKUPS_DIR` references: 7 in main → 0 in the PR);
restore becomes a manual action. Report this entry should then read "fail-closed + manual restore".
